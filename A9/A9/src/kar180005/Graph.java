...
public class Graph implements Iterable<Graph.Vertex> {
	...
	/**
	 * Method to go through edges incident at vertex u. Can be used with
	 * implicit/explicit iterators
	 */
	...
	public void printGraph(boolean hasEdgeWeights) {
		...
		System.out.println(
				"Graph: n: " + size() + ", m: " + edgeSize() + ", directed: " + directed + ", Edge weights: " + hasEdgeWeights);
		for (Vertex u : this) {
			System.out.print(u + " : ");
	...
	/**
	 * Interface used by classes that store properties of vertices during graph
	 * algorithms
	 */
	...
	// This method is used when a graph has been cloned to get g's vertex from its
	// clone
	...
	public class AdjList {
		...
		public AdjList(int n) { // create an empty AdjList entry for Vertex named n
			vertex = new Vertex(n);
			outEdges = new LinkedList<>(); // incident edges of vertex in undirected graph, or its outgoing edges in a
																			// directed graph
			inEdges = new LinkedList<>(); // empty list if undirected graph; list of incoming edges at vertex if directed
																		// graph
		}
	}
	...
	/**
	 * Class to store properties of vertices in a class V that implements Factory. A
	 * parallel array is created to store properties of all vertices. Class
	 * GraphAlgorithm creates a Store from its constructor.
	 */
	...
	/**
	 * Class to facilitate writing graph algorithms, where a generic class V is used
	 * to store attributes of vertices during the algorithm.
	 */
	public static class GraphAlgorithm<V extends Factory> {
		...
		/**
		 * @param: a graph g and a sample node vf used to create additional nodes using
	...
	/**
	 * Iterator class to iterate over an array or subarray. Remove is not
	 * implemented.
	 */
